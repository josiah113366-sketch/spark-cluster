...
spark = (SparkSession.builder 
  ...
  .master('spark://spark-master:7077')        # 도커 컴포즈 상에 존재하는 마스터의 주소 
  .config("spark.executor.memory","512m")     # 워커들의 메모리
  .config("spark.executor.cores","1")         # 워커들의 코어 수
  .config("spark.sql.shuffle.partitions","8") # 셔플 파티션 8개 준비
  .getOrCreate()
)

# 로그 레벨 지정
spark.sparkContext.setLogLevel("WARN")        # 경고 위의 레벨까지 로그 출력 
...
.option("header", True) \
.schema(schema) \
.csv('/opt/data/hospital_raw.csv') # 차후 이 지점은 s3로 대체 
...
# 2개의 worker에서 자원을 쪼개서 각각 2개의 executor를 생성하여 총 4개의 executor들이 작업을 해서 show() 처리를 수행함 
